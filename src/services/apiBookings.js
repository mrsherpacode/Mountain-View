...
export async function getBookings({ filter, sortBy, page } = {}) {
  // here, the second arguement count gets the exact count of the data
  let query = supabase
    .from("bookings")
...
//  Getting one booking for booking details
...
export async function getBookingsAfterDate(date) {
  const { data, error } = await supabase
    ...
    .gte("created_at", date) // from start date
    .lte("created_at", getToday({ end: true })); // to end of today
...
export async function getStaysTodayActivity() {
  const { data, error } = await supabase
    .from("bookings")
    .select("*, guests(fullName, nationality, countryFlag:countyFlag)")
    .or(
